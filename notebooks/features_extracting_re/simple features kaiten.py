...
def main():
    df = pd.read_csv('./kaiten_preprocessed/kaiten_preprocessed.csv').dropna().reset_index(drop=True).sort_values('Резюме')
    # Колонки под фичи
    feature_columns = [
    ...
    # Сохраняем
    result_df.to_csv('./simple_features_kaiten.csv', index=False)
    print("Готово! Файл simple_features_kaiten.csv сохранён.")
