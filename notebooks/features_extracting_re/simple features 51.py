...
def main():
    df = pd.read_csv('./51_preprocessed/51_preprocessed.csv').dropna().reset_index(drop=True).sort_values('Резюме')
    glob_path = "51_preprocessed/Резюме/*"
    # Колонки под фичи
    feature_columns = [
    ...
    for i, (pdf_path, resume_name) in enumerate(tqdm(zip(sorted([i.replace('/', '\\') for i in glob(glob_path)]), df['Резюме']), total=len(df))):
        pdf_file = resume_name.replace(' ', '_').replace('__', '_')
        if pdf_file!=pdf_path:
    ...
    # Сохраняем
    result_df.to_csv('./simple_features_51.csv', index=False)
    print("Готово! Файл simple_features_51.csv сохранён.")
